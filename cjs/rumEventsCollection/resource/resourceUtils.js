...
exports.FAKE_INITIAL_DOCUMENT = void 0;
...
var RESOURCE_TYPES = [[_enums.ResourceType.DOCUMENT, function (initiatorType) {
  return FAKE_INITIAL_DOCUMENT === initiatorType;
}], [_enums.ResourceType.XHR, function (initiatorType) {
  return 'xmlhttprequest' === initiatorType;
}], [_enums.ResourceType.FETCH, function (initiatorType) {
  return 'fetch' === initiatorType;
}], [_enums.ResourceType.BEACON, function (initiatorType) {
  return 'beacon' === initiatorType;
}], [_enums.ResourceType.CSS, function (_, path) {
  return path.match(/\.css$/i) !== null;
}], [_enums.ResourceType.JS, function (_, path) {
  return path.match(/\.js$/i) !== null;
}], [_enums.ResourceType.IMAGE, function (initiatorType, path) {
  return (0, _tools.includes)(['image', 'img', 'icon'], initiatorType) || path.match(/\.(gif|jpg|jpeg|tiff|png|svg|ico)$/i) !== null;
}], [_enums.ResourceType.FONT, function (_, path) {
  return path.match(/\.(woff|eot|woff2|ttf)$/i) !== null;
}], [_enums.ResourceType.MEDIA, function (initiatorType, path) {
  return (0, _tools.includes)(['audio', 'video'], initiatorType) || path.match(/\.(mp3|mp4)$/i) !== null;
}]];

function computeResourceKind(timing) {
  var url = timing.name;

  if (!(0, _urlPolyfill.isValidUrl)(url)) {
    return _enums.ResourceType.OTHER;
  }

  var path = (0, _urlPolyfill.getPathName)(url);
  var type = _enums.ResourceType.OTHER;
  (0, _tools.each)(RESOURCE_TYPES, function (res) {
    var _type = res[0],
        isType = res[1];
...
function areInOrder() {
  var numbers = (0, _tools.toArray)(arguments);
...
function computePerformanceResourceDuration(entry) {
  ...
  if (entry.duration === 0 && entry.startTime < entry.responseEnd) {
    return (0, _tools.msToNs)(entry.responseEnd - entry.startTime);
  }
...
function formatTiming(origin, start, end) {
  return (0, _tools.msToNs)(end - start); // return {
