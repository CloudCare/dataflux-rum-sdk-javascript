...
export var report = function reportModuleWrapper() {
  ...
  function traceKitWindowOnError(message, url, lineNo, columnNo, errorObj) {
    ...
    if (oldOnerrorHandler) {
      return oldOnerrorHandler.apply(_this, arguments);
    }
  ...
  function doReport(ex) {
    ...
    lastException = ex; // If the stack trace is incompvare, wait for 2 seconds for
    // slow slow IE to see if onerror occurs or not before reporting
    // this exception; otherwise, we will end up with an incompvare
    // stack trace
