...
export function startRumAssembly(applicationId, configuration, lifeCycle, session, parentContexts, getGlobalContext) {
  lifeCycle.subscribe(LifeCycleEventType.RAW_RUM_EVENT_V2_COLLECTED, function (data) {
    ...
    if (session.isTracked() && viewContext && viewContext.session.id) {
      ...
      var rumEvent = needToAssembleWithAction(rawRumEvent) ? extend2Lev(rumContext, deviceContext, viewContext, actionContext, rawRumEvent) : extend2Lev(rumContext, deviceContext, viewContext, rawRumEvent);
      var serverRumEvent = withSnakeCaseKeys(rumEvent);
      serverRumEvent.context = extend2Lev(savedGlobalContext || getGlobalContext(), customerContext);
