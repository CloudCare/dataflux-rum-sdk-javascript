...
import { addEventListener, extend, UUID, noop, ONE_MINUTE, throttle } from '../../helper/tools';
...
function trackActivityLoadingTime(lifeCycle, callback) {
  ...
  var _waitIdlePageActivity = waitIdlePageActivity(lifeCycle, function (hadActivity, endTime) {
    if (hadActivity) {
...
function trackLayoutShift(lifeCycle, callback) {
  var _lifeCycle = lifeCycle.subscribe(LifeCycleEventType.PERFORMANCE_ENTRY_COLLECTED, function (entry) {
